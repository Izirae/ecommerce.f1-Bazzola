	...
	const items = cart.map((item) => {
		return (
			<Tr key={item.id}>
				...
				<Td><Button onClick={() => removeItem(item.id)} colorScheme="red" size={"sm"}><Icon as={RiDeleteBin2Fill} w={6} h={6} /></Button></Td>
			</Tr>
		);
	});
			...
			<>
				<TableContainer>
					<Table variant="striped" align='center' borderWidth={'2px'} colorScheme="teal" w={'75%'}>
						<TableCaption><Heading size={'md'}>Total: ${finalPrice()} USD</Heading></TableCaption>
						<Thead>
							<Tr>
								<Th>Img</Th>
								<Th>Producto</Th>
				...
				<Center mt={5}>
					<Button onClick={() => clear()} colorScheme="red" size={"sm"}><Icon as={RiDeleteBin2Fill} w={6} h={6} />Vaciar carrito</Button>
				</Center>
			</>)
	);
}
